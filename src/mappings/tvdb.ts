...
export const getTVDBInfo = async (
  id: string,
  year?: string,
  format: string = "TV"
  ...
  try {
    ...
    console.log("TheTVDB mapping not found for ID: ", id);
    return null;
  }
};

export const getTVDB = async (id: string) => {
  try {
    const ani = new Anilist();
    const tv = new TheTVDB();
    ...
    if (response?.length! <= 0) {
      ...
      const data = await res.json<
        { mal_id: number; anilist_id: number; thetvdb_id: number }[]
      >();
      const malId = info?.idMal;

      const matched = data.find(
        (id) => Number(id.mal_id) === Number(malId)
      )?.thetvdb_id;

      const tvInfo = await getTVDBInfo(
        String(matched)!,
        String(info?.startDate?.year) || undefined,
        info?.format || undefined
      );
